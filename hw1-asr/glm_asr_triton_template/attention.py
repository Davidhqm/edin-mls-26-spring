...
class FusedQKVLinear:
    def __init__(self, hidden_size, num_heads, num_kv_heads):
        ...
        self.out_features = (num_heads + 2 * num_kv_heads) * self.d_head
        self.weight = torch.empty((self.out_features, hidden_size), device='cuda', dtype=torch.float16)

    def __call__(self, x):
        M = int(np.prod(x.shape[:-1]))
        K, N = x.shape[-1], self.out_features
        out = torch.empty((M, N), device=x.device, dtype=torch.float32)
        
        # Grid launch using Hopper-safe tiles
        grid = (triton.cdiv(M, 128), triton.cdiv(N, 64))
        fused_qkv_kernel[grid](
            x, self.weight.t().contiguous(), out,
            M, N, K,
            x.stride(0), x.stride(1), K, 1, N, 1,
            BLOCK_M=128, BLOCK_N=64, BLOCK_K=32
        )
        return out.reshape(*x.shape[:-1], -1, self.d_head)
